...
//bucket is a linked in each slot of the hash table array
type bucket struct {
	head *bucketNode
}
...
//Search will take in a key and reture true if that key is stored in the hash table
func (h *HashTable) Search(key string) bool {
	index := hash(key)
	return h.array[index].search(key)
}

//Delete will take in a key and delete from the hash table
...
func main() {
	...
	/*
		...
		testBucket.insert("RANDY321")
		fmt.Println(testBucket.search("RANDY"))
		fmt.Println(testBucket.search("RANDY321"))
	*/
	hashTable := Init()
	list := []string{
		"eric",
	...
	fmt.Println("butters", hashTable.Search("butters"))
